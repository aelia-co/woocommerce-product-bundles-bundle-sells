	...
	*/
	...
	public static function display_bundle_sells() {
		...
		if ( ! empty( $bundle_sell_ids ) ) {
			...
			wp_register_style( 'wc-pb-bs-single', WC_PB_Bundle_Sells::plugin_url() . '/assets/css/wc-pb-bs-single.css', false, WC_PB_Bundle_Sells::$version, 'all' );
			...
			if ( $bundle_sells_title ) {
				...
				), false, WC_PB_Bundle_Sells::plugin_path() . '/templates/' );
			}
	...
	public static function get_bundled_item_template_location( $template, $template_name, $template_path ) {
		...
		if ( false === strpos( $template_path, WC_PB_Bundle_Sells::plugin_path() ) ) {
			if ( 'single-product/bundled-item-quantity.php' === $template_name ) {

				$template = wc_locate_template( 'single-product/bundle-sell-quantity.php', '', WC_PB_Bundle_Sells::plugin_path() . '/templates/' );
				...
				if ( $template_name_override !== $template_name ) {
					$template = wc_locate_template( $template_name_override, '', WC_PB_Bundle_Sells::plugin_path() . '/templates/' );
				}
			}
		}
